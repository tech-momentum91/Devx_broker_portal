...
export const buttonVariants = tv({
  slots: {
    root: [
      // base
      'group relative cursor-pointer inline-flex items-center justify-center whitespace-nowrap outline-none',
  ...
  variants: {
    ...
    size: {
      medium: { root: 'h-10  rounded-10 py-[12px] px-[10px] text-label-sm', icon: '-mx-1' },
      small: { root: 'h-9 rounded-lg px-3 text-label-sm', icon: '-mx-1' },
      xsmall: { root: 'h-8  rounded-lg px-2.5 text-label-sm', icon: '-mx-1' },
      xxsmall: { root: 'h-7  rounded-lg px-2 text-label-sm', icon: '-mx-1' },
    },
  },
  compoundVariants: [
    // variant = primary
...
const ButtonRoot = React.forwardRef(
  ({ children, variant, mode, size, asChild, className, ...rest }, forwardedRef) => {
    ...
    const sharedProps = { variant, mode, size };
